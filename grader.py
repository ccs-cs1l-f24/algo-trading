...
class Wallet:
    ...
    def get_total_worth(self, stock_prices):
        print("total worth: $" + str(self.get_worth_of_portfolio(stock_prices)/100 + self.total_funds/100))
        print("total value of assets: $" + str(self.get_worth_of_portfolio(stock_prices)/100))
        print("total cash: $" + str(self.total_funds/100))
        return
    ...
    def place_orders(self, stock_counts, stock_prices):
        ...
        for stock_symbol in stock_counts.keys():
            if stock_counts[stock_symbol] < 0:  #sell shares
                ...
                self.portfolio[stock_symbol] += stock_counts[stock_symbol]
                self.total_funds -= stock_counts[stock_symbol] * stock_prices[stock_symbol]
                # print("bought", stock_symbol, stock_counts[stock_symbol], stock_prices[stock_symbol], "total funds:", self.total_funds)